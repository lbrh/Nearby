...
import type { Lang, NearbyResponse } from "@/app/lib/types";
import { RESULT_COPY } from "@/app/lib/i18n";
...
export default function Results({
  ...
  const rc = RESULT_COPY[lang] ?? RESULT_COPY.en;
  const chips = [suburb, demographic].filter(Boolean);

  return (
    <section className="screen-fade flex flex-1 flex-col gap-[18px]">
        ...
        <div className="flex-1">
          <div className="mb-2.5 flex flex-wrap items-center gap-2 font-mono text-[10.5px] uppercase tracking-[0.14em] text-ink-3">
            {chips.map((c) => (
              <span
                key={c}
                className="rounded-full border border-line bg-paper px-2.5 py-1 text-ink-2"
              >
                {c}
              </span>
            ))}
          </div>
          <h2 className="m-0 mb-1.5 text-balance font-display text-[clamp(28px,5.6vw,38px)] leading-[1.1] tracking-[-0.015em]">
            {rc.title}
          </h2>
